...
{
public:
	explicit BlockingReaderWriterCircularBuffer(std::size_t capacity,std::size_t typesize)
		...
		nextSlot(0), nextItem(0)
	{
		...
		mask = capacity++;
		rawData = static_cast<char*>(std::malloc(capacity * typesize + 20 - 1));
...
private:
	...
	void inner_enqueue(U&& item)
	{
		...
		new (reinterpret_cast<U*>(rawData) + (i & mask)) U(std::forward<U>(item));
		items->signal();
	}
	...
	void inner_dequeue(U& item)
	{
		...
		U& element = reinterpret_cast<U*>(rawData)[i & mask];
	...
	T* inner_peek()
	{
		return reinterpret_cast<T*>(rawData) + (nextItem & mask);
	}
	template<typename T>
	void inner_pop()
	{
		std::size_t i = nextItem++;
		reinterpret_cast<T*>(rawData)[i & mask].~T();
		slots_->signal();
	}
